...
function ReportsPage() {
  ...
  const runPreset = async (key: string, fn: () => Promise<boolean>) => {
    setBusy(key);
    try {
      if (await fn()) toast.success("הדוח הופק");
    } catch (e) {
          ...
          <div key={p.key} className="card-surface p-5 flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-2xs font-bold px-1.5 py-0.5 rounded bg-muted text-muted-foreground">
                  {p.format}
                </span>
              </div>
              <p className="text-xs text-muted-foreground mt-1">{p.desc}</p>
              <button
                onClick={() => runPreset(p.key, p.run)}
                disabled={busy !== null}
      ...
      <div className="mt-5 card-surface p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <StackedField label="פורמט">
            <select
              value={fmt}
              onChange={(e) => setFmt(e.target.value as "pdf" | "xlsx")}
              className="field-input w-full"
            >
