...
function FrameworkPanel({ fw, enabled }: { fw: LearningFramework; enabled: boolean }) {
  ...
  const limitReached =
    isMine && timer!.limitMinutes !== undefined && elapsedMs / 60000 >= timer!.limitMinutes;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="card-surface p-5 lg:col-span-2 space-y-4">
        {isMine && (
          <div
            className={`rounded-lg border-2 p-4 ${paused ? "border-border bg-muted/40" : "border-primary/40 bg-primary/5"}`}
          >
            ...
            <div
              className={`text-3xl font-bold tabular-nums ${paused ? "text-muted-foreground" : ""}`}
            >
            ...
            {timer!.limitMinutes !== undefined && (
              <div className="mt-1 text-2xs text-muted-foreground">
                מוגבל ל־{timer!.limitMinutes} דק׳ · נותרו{" "}
                {Math.max(0, timer!.limitMinutes - elapsedMin)} דק׳
              </div>
            )}
            {timer!.tanitDibur && (
        ...
        {fw === "kollel-erev" && (
          <label className="flex items-center gap-2 text-xs cursor-pointer rounded-lg border border-border p-3">
            <input
              type="checkbox"
              checked={tanitDibur}
              onChange={(e) => setTanitDibur(e.target.checked)}
            />
            <MicOff className="size-3.5" />
            לימוד בבית בתענית דיבור{" "}
            <span className="text-muted-foreground">
              (כל דקה נחשבת כפול בסיכום — חל על הוספה ידנית, לפי טווח שעות וטיימר)
            </span>
          </label>
        )}
              ...
              {limitOn && (
                <div className="flex items-center gap-2 pe-6">
                  ...
                  <span className="text-xs text-muted-foreground">
                    דקות · הטיימר ייעצר וישמר אוטומטית
                  </span>
                </div>
              )}
            </div>
      ...
      <div className="card-surface p-5">
        ...
        <div className="text-3xl font-bold tabular-nums mt-1">
          {(effectiveTotalMin / 60).toFixed(1)}{" "}
          <span className="text-sm text-muted-foreground">שע׳</span>
        </div>
        <div className="text-xs text-muted-foreground mt-1">
          {effectiveTotalMin} דקות · {fwItems.length} רישומים החודש
        </div>
        {tanitMin > 0 && (
          <div className="mt-2 rounded-md bg-primary/5 border border-primary/20 p-2 text-2xs text-primary flex items-start gap-1.5">
            ...
            <span>
              נלמדו {tanitMin} דק׳ בתענית דיבור · נחשב כ־{tanitMin * 2} דק׳
            </span>
          </div>
        )}
    ...
    <AppShell title="לימוד נוסף" subtitle="מסגרות לימוד מחוץ לסדרים">
      <div className="card-surface p-1 mb-4 inline-flex gap-1">
        {FRAMEWORKS.map((fw) => (
          ...
          >
            {FRAMEWORK_LABELS[fw]}
            {fw === "bein-hazmanim" && !beinHaz && (
              <span className="ms-1 text-2xs opacity-60">(לא בעונה)</span>
            )}
          </button>
        ))}
      </div>
