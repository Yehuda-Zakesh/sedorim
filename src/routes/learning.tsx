...
type Item = { id: string; topic: string; date: string; minutes: number };
...
function LearningPage() {
  ...
  const add = () => {
    ...
    setItems([{ id: Date.now().toString(), topic, date: new Date().toISOString().slice(0, 10), minutes }, ...items]);
  ...
  return (
    <AppShell title="לימוד נוסף" subtitle="שיעורים ותגבורים אישיים">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        {[
          { label: "סה״כ שעות", value: totalHr, icon: Clock4 },
          { label: "מספר שיעורים", value: items.length.toString(), icon: BookOpen },
          { label: "החודש", value: "5", icon: BookOpen },
        ].map((k) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="card-surface p-5">
          ...
          <input
            ...
            className="mt-1 w-full rounded-md border border-input bg-card px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
          />
          <button onClick={add} className="mt-4 w-full inline-flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90">
