        ...
        <div className="card-surface p-5 lg:col-span-3">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-1">
              <button
                onClick={() => goto(-1)}
                className="size-9 rounded-md hover:bg-accent grid place-items-center"
              >
            ...
            <button
              ...
              className="text-xs rounded-md border border-border px-3 py-1.5 hover:bg-accent"
            >
              היום
            </button>
          </div>
                    ...
                    list.length
                      ? {
                          ...
                          borderColor: isSelected
                            ? "var(--color-primary)"
                            : `color-mix(in oklch, ${t.color} 50%, transparent)`,
                        }
                      : undefined
                  }
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-xs font-semibold tabular-nums ${isToday ? "text-primary" : ""}`}
                    >
                      {day}
                    </span>
                  </div>
                  {list.length > 0 && (
                    <div
                      className="mt-auto self-end size-2 rounded-full"
                    ...
                    return (
                      <li key={e.id} className="rounded-md border border-border p-3">
                        ...
                        <div className="text-2xs text-muted-foreground mt-1">
                          חסר {c.netMissingMin} · בונוס {c.bonusMin}
                          {c.isOhevei && " · אוהבי ה׳"}
                        </div>
                      </li>
                    );
