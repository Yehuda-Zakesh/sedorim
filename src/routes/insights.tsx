...
import { useAttendance, useLearning, attendanceRate, currentStreak } from "@/lib/tracker-store";
import { useSettings } from "@/lib/settings-store";
import { generateInsights, forecastMonthRate, consistencyScore, type Insight } from "@/lib/insights";
...
function InsightsPage() {
  ...
  const insights = generateInsights(records, lessons, settings.goals);
  const forecast = forecastMonthRate(records);
  const consistency = consistencyScore(records);
  ...
  return (
    <AppShell title="תובנות חכמות" subtitle="ניתוח אוטומטי של הנתונים שלך">
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        <KpiCard label="ממוצע כולל"    value={`${overall}%`}   icon={Target} />
        <KpiCard label="רצף נוכחי"      value={streak.toString()} icon={Sparkles} />
        <KpiCard label="תחזית לחודש"   value={forecast !== null ? `${forecast}%` : "—"} icon={TrendingUp} />
      ...
      {insights.length === 0 ? (
        <div className="card-surface p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">המשך לרשום נוכחות כדי לקבל תובנות מותאמות.</p>
        </div>
      ) : (
          ...
          {(["trend", "opportunity", "recommendation"] as const).map((cat) => (
            <div key={cat} className="card-surface p-5">
              ...
              {grouped[cat].length === 0 ? (
                <p className="text-xs text-muted-foreground">אין כרגע</p>
              ) : (
