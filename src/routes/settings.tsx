...
import {
  ...
  removeSederOverride,
  sederTimesError,
  SHAS_ARRIVAL_DEADLINE,
  type SederTimes,
} from "@/lib/settings-store";
import { COLOR_THEMES, BG_THEMES } from "@/lib/theme-colors";
import {
  announce,
  useNotificationLearning,
  resetNotificationLearning,
  NOTIFICATION_KINDS,
  type NotificationKind,
} from "@/lib/notifications";
import type { KindMemory } from "@/lib/notification-learning";
import { isRegisteredToStartWithWindows, setBackgroundEnabled } from "@/lib/background";
...
const SECTIONS = [
  {
    ...
    label: "שעות הסדרים",
    icon: Clock,
    hint: "מתי מתחיל ומסתיים כל סדר, שינויים לתקופה, וחבורת ש״ס",
  },
  {
    id: "goals",
    label: "יעדים והתראות",
    icon: Target,
    hint: "יעד חודשי, מכסת איחורים ואיזה תזכורות להציג",
  },
  ...
  { id: "appearance", label: "מראה ועיצוב", icon: Palette, hint: "צבעים, רקע, גודל גופן" },
  {
    ...
    hint: "אילו חלקים להציג במסך הראשי",
  },
  {
    id: "data",
    label: "נתונים וגיבוי",
    icon: Database,
    hint: "גיבוי אוטומטי ומספר הגיבויים לשמור",
  },
                ...
                <div className="px-5 pb-5 border-t border-border pt-4 space-y-3">
                  {s.id === "seder" && (
                    <>
                      <SederHoursManager />
                      <NumberField
                        label="סף בונוס להגעה מוקדמת (דק׳)"
                        min={0}
                        max={60}
                        value={settings.seder.bonusThresholdMin}
                        onChange={(v) =>
                          update({ seder: { ...settings.seder, bonusThresholdMin: v } })
                        }
                      />
                      <p className="text-2xs text-muted-foreground">
                        הגעה לפני תחילת הסדר נצברת כבונוס שמקטין את החסר — עד למספר הדקות הזה בכל
                        סדר.
                      </p>
                        ...
                        label={
                          <span>
                            ...
                            <span className="block text-2xs text-muted-foreground">
                              המערכת תספור כל הגעה לסדר ב׳ עד {SHAS_ARRIVAL_DEADLINE}, ותציג את
                              המספר בכל מסכי הסיכום, בדוחות ובמחשבון המלגה.
                            </span>
                          </span>
                        }
                      ...
                      />
                      <p className="text-2xs text-muted-foreground">
                        הספירה נעשית מהרישומים הקיימים — סימון החבורה עכשיו מציג גם את כל ההגעות
                        שכבר נרשמו, בלי להזין דבר מחדש.
                      </p>
                    </>
                  )}
                  ...
                  {s.id === "goals" && (
                    <>
                      ...
                      />
                      <NumberField
                        label="מקסימום איחורים בחודש"
                        min={0}
                        max={31}
                        value={settings.goals.maxLatePerMonth}
                        onChange={(v) =>
                          update({ goals: { ...settings.goals, maxLatePerMonth: v } })
                        }
                      />
                      <NumberField
                        label="סף התראה לדקות חסרות בחודש"
                        min={0}
                        max={1440}
                        value={settings.seder.alertMissingMinPerMonth}
                        onChange={(v) =>
                          update({ seder: { ...settings.seder, alertMissingMinPerMonth: v } })
                        }
                      />

                      <div className="pt-2 text-xs font-semibold text-muted-foreground">
                        איך להציג את התזכורות
                      </div>
                      <Toggle
                        label="הודעות קופצות בתוך התוכנה"
                        on={settings.notifications.popups}
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, popups: v } })
                        }
                      />
                      <Toggle
                        label={
                        ...
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, desktop: v } })
                        }
                      />
                      ...
                      <Toggle
                        label="תזכורת יומית — כשלא נרשם סדר עד תחילת סדר א׳"
                        on={settings.notifications.dailyReminder}
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, dailyReminder: v } })
                        }
                      />
                      <Toggle
                        label="התראה בחריגה ממכסת האיחורים החודשית"
                        on={settings.notifications.latenessAlert}
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, latenessAlert: v } })
                        }
                      />
                      <Toggle
                        label="סיכום שבועי"
                        on={settings.notifications.weeklySummary}
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, weeklySummary: v } })
                        }
                      />
                      <Toggle
                        label={
                          <span>
                            ...
                            <span className="block text-2xs text-muted-foreground">
                              פעם בחודש, כשלפי הקצב עד כה החודש צפוי לעבור את סף הדקות החסרות — בזמן
                              שעוד אפשר לסגור את הפער
                            </span>
                          </span>
                        }
                        ...
                        onChange={(v) =>
                          update({
                            notifications: { ...settings.notifications, forecastWarning: v },
                          })
                        }
                      />
                      <Toggle
                        label={
                          <span>
                            תזכורת לדיווח במערכת הטלפונית
                            <span className="block text-2xs text-muted-foreground">
                              בימים 1–5 בחודש, בשעה 20:00, על החודש שהסתיים — ונפסקת ברגע שמסמנים
                              שדיווחת
                            </span>
                          </span>
                        }
                        on={settings.notifications.phoneReport}
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, phoneReport: v } })
                        }
                      />
                      ...
                      <Toggle
                        ...
                        onChange={(v) =>
                          update({ notifications: { ...settings.notifications, adaptive: v } })
                        }
                      />
                  ...
                  {s.id === "profile" && (
                    <>
                      ...
                      <Field
                        label="כולל / קבוצה"
                        value={settings.profile.classroom}
                        onChange={(v) => update({ profile: { ...settings.profile, classroom: v } })}
                      />
                      <p className="text-2xs text-muted-foreground">
                        שני השדות האלה מופיעים בכותרת כל דוח שמופק.
                      </p>
                    </>
                  )}
                  ...
                  {s.id === "appearance" && (
                    <>
                      <ColorThemePicker
                        ...
                        onChange={(v) =>
                          update({ appearance: { ...settings.appearance, colorTheme: v } })
                        }
                      />
                      <BackgroundPicker
                        value={settings.appearance.background}
                        onChange={(v) =>
                          update({ appearance: { ...settings.appearance, background: v } })
                        }
                      />
                      <SelectField
                        label="גודל גופן"
                        ...
                        onChange={(v) =>
                          update({
                            appearance: { ...settings.appearance, fontSize: v as FontSize },
                          })
                        }
                      />
                      <Toggle
                        label={
                          <span className="inline-flex items-center gap-2">
                            <Contrast className="size-4" /> ניגודיות גבוהה
                          </span>
                        }
                        on={settings.appearance.highContrast}
                        onChange={(v) =>
                          update({ appearance: { ...settings.appearance, highContrast: v } })
                        }
                      />
                      <Toggle
                        label="תצוגה צפופה"
                        ...
                        onChange={(v) =>
                          update({ appearance: { ...settings.appearance, compactMode: v } })
                        }
                      />
                    </>
                  )}
                  ...
                  {s.id === "dashboard" && (
                    <>
                      <Toggle
                        label="הצג סיכום מהיר ותובנות"
                        on={settings.dashboard.showInsights}
                        onChange={(v) =>
                          update({ dashboard: { ...settings.dashboard, showInsights: v } })
                        }
                      />
                      <Toggle
                        label="הצג תזכורות"
                        on={settings.dashboard.showReminders}
                        onChange={(v) =>
                          update({ dashboard: { ...settings.dashboard, showReminders: v } })
                        }
                      />
                      <Toggle
                        label="הצג פעולות מהירות"
                        on={settings.dashboard.showQuickActions}
                        onChange={(v) =>
                          update({ dashboard: { ...settings.dashboard, showQuickActions: v } })
                        }
                      />
                    </>
                  )}
                  ...
                  {s.id === "data" && (
                    <>
                      <SelectField
                        ...
                        onChange={(v) =>
                          update({
                            data: { ...settings.data, autoBackup: v as "off" | "daily" | "weekly" },
                          })
                        }
                      />
                      <NumberField
                        label="מספר גיבויים לשמור"
                        min={1}
                        max={20}
                      ...
                      />
                      <Toggle
                        label="גיבוי לפני פעולות גדולות"
                        on={settings.data.autoBackupBeforeOps}
                        onChange={(v) =>
                          update({ data: { ...settings.data, autoBackupBeforeOps: v } })
                        }
                      />
                      <Link
                        to="/backup"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground truncate">
                            ייצוא, ייבוא, תמונות מצב ומחיקת נתונים
                          </div>
                        </div>
...
const KIND_LABELS: Record<NotificationKind, string> = {
  ...
  "phone-report": "דיווח למערכת הטלפונית",
};
...
function AdaptiveStatus() {
  ...
  const rows = NOTIFICATION_KINDS.map((kind) => ({ kind, memory: learning[kind] })).filter(
    (r): r is { kind: NotificationKind; memory: KindMemory } => (r.memory?.delivered ?? 0) > 0,
  );
...
/**
 * "תזכורות גם כשהתוכנה סגורה".
 *
 * The one switch in the app that starts a process, so it says plainly what it
 * starts, what that costs, and what it is allowed to say — and it checks the
 * login entry against Windows rather than trusting the saved flag, because
 * the two can disagree and the difference is a morning without a reminder.
 */
function BackgroundToggle() {
  const { settings } = useSettings();
  const enabled = settings.background.enabled;
  const [busy, setBusy] = useState(false);
  const [registered, setRegistered] = useState<boolean | null>(null);

  const refreshRegistration = () => {
    void isRegisteredToStartWithWindows().then(setRegistered);
  };
  useEffect(refreshRegistration, [enabled]);
  ...
  return (
    <div className="rounded-lg border border-border p-4">
      <Toggle
        label={
          <span>
            ...
            <span className="block text-2xs text-muted-foreground">
              מפעיל תוכנית רקע קטנה (SederPlusAgent) שנטענת עם Windows. אין לה חלון ואין לה אייקון —
              היא תופסת כמה מגה־בייט בלבד, לעומת כ־150MB שהתוכנה עצמה תופסת.
            </span>
          </span>
        }
        on={enabled}
        onChange={async (v) => {
          setBusy(true);
          try {
            const result = await setBackgroundEnabled(v);
            if (!result.ok) toast.error("הפעלת מצב הרקע נכשלה — פרטים ביומן התקלות");
            else if (v) toast.success("מצב הרקע פועל. אפשר לסגור את התוכנה.");
            else toast.success("מצב הרקע כובה");
          } finally {
            setBusy(false);
            refreshRegistration();
          }
      ...
      <p className="mt-2 text-2xs text-muted-foreground">
        ברקע נשלחות שתי תזכורות בלבד: שסדר התחיל ועדיין לא נרשם, ושצריך לדווח במערכת הטלפונית על
        החודש שעבר. כל השאר מופיע רק כשהתוכנה פתוחה.
      </p>
      {enabled && registered === false && (
        <p className="mt-2 text-2xs text-destructive">
          התוכנית פועלת עכשיו, אבל היא לא רשומה לעלות עם Windows. כבה את המתג והדלק אותו שוב.
        </p>
      )}
      {busy && <p className="mt-2 text-2xs text-muted-foreground">רגע…</p>}
    </div>
  );
}
...
function NotificationTester() {
  ...
  return (
    <div className="rounded-lg border border-border p-4">
      ...
      <p className="mt-1 text-2xs text-muted-foreground">
        נשלחת התראה לדוגמה בכל הערוצים שסימנת. אם התראת שולחן העבודה לא מופיעה, בדוק ב"הגדרות
        Windows ← מערכת ← התראות" שההתראות עבור סדר פלוס מופעלות.
      </p>
      <button
        disabled={busy}
        onClick={async () => {
          ...
          try {
            ...
            if (result.desktop) toast.success("נשלחה התראת שולחן עבודה");
              ...
              toast.success("הודעה קופצת פועלת. התראות שולחן העבודה כבויות או חסומות.");
...
function ProblemLog() {
  ...
  return (
    <>
      ...
      <div
        className="rounded-lg border border-border bg-muted/30 max-h-72 overflow-auto p-3"
        dir="ltr"
      >
        {loading ? (
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {isDesktop && (
          <button
            onClick={async () => {
              if (!(await openLogFolder())) toast.error("פתיחת התיקייה נכשלה");
            }}
            className="inline-flex items-center gap-2 rounded-md border border-border px-3 py-1.5 text-xs font-medium hover:bg-accent"
          >
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      <TimeField
        label="סדר א׳ — תחילה"
        value={times.s1Start}
        onChange={(v) => onChange({ ...times, s1Start: v })}
      />
      <TimeField
        label="סדר א׳ — סיום"
        value={times.s1End}
        onChange={(v) => onChange({ ...times, s1End: v })}
      />
      <TimeField
        label="סדר ב׳ — תחילה"
        value={times.s2Start}
        onChange={(v) => onChange({ ...times, s2Start: v })}
      />
      <TimeField
        label="סדר ב׳ — סיום"
        value={times.s2End}
        onChange={(v) => onChange({ ...times, s2End: v })}
      />
    </div>
  );
}
...
function SederHoursManager() {
  ...
  const schedule = [...(settings.sederSchedule || [])].sort((a, b) =>
      ...
      <div className="rounded-lg border border-border p-4 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            onClick={() => {
              setSederTimesFromToday(draft, from);
              toast.success(`השעות עודכנו מתאריך ${from} ואילך`);
            }}
            ...
            {schedule.map((e) => (
              <li key={e.id} className="flex items-center justify-between gap-3 text-xs">
                ...
                <span className="tabular-nums">
                  {e.times.s1Start}–{e.times.s1End} · {e.times.s2Start}–{e.times.s2End}
                </span>
                <button
                  onClick={() => {
      ...
      <div className="rounded-lg border border-border p-4 space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-2xs text-muted-foreground">
              בתום הטווח השעות חוזרות אוטומטית להגדרה השמורה.
            </div>
          </div>
          <button
          ...
          <ul className="space-y-2 border-t border-border pt-3">
            {overrides.map((o) => (
              <li key={o.id} className="flex items-center justify-between gap-3 text-xs">
                ...
                <span className="tabular-nums">
                  {o.times.s1Start}–{o.times.s1End} · {o.times.s2Start}–{o.times.s2End}
                </span>
                <button
                  onClick={() => {
                    const before = settings.sederOverrides || [];
                    removeSederOverride(o.id);
                    toastUndo("השינוי הזמני נמחק", () =>
                      updateSettings({ sederOverrides: before }),
                    );
                  }}
                  className="text-muted-foreground hover:text-destructive"
                >
  ...
  return (
    <div>
      ...
      <div
        ...
        className="grid grid-cols-6 sm:grid-cols-11 gap-2.5"
      >
        {COLOR_THEMES.map((t) => {
  ...
  return (
    <div>
      ...
      <div
        role="radiogroup"
        aria-label="רקע מסך"
        className="grid grid-cols-5 sm:grid-cols-10 gap-2.5"
      >
        {BG_THEMES.map((t) => {
          const active = value === t.id;
          return (
