...
מעקב נוכחות ולימוד לכולל. שלוש תוכנות ל‑Windows, בסיס קוד אחד:

| קובץ EXE             | מה זה                          |
| -------------------- | ------------------------------ |
| `SederPlus.exe`      | האפליקציה המלאה                |
| `SederPlusQuick.exe` | חלון ההגעה הקטן שתמיד בהישג יד |
| `SederPlusAgent.exe` | תזכורות כשהשתיים האחרות סגורות |

שתי הראשונות הן **קובץ בודד עצמאי** במשקל כ‑10MB כל אחת. הממשק מוצג דרך רכיב
WebView2 שמגיע עם Windows 11 (וקיים כמעט בכל התקנה של Windows 10).

השלישית היא אחרת לגמרי: אין לה חלון, אין לה אייקון ואין לה WebView — כ‑300KB
שישנים כל היום ומעירים שתי תזכורות. היא כבויה עד שמבקשים אותה, ורצה על כ‑2MB
זיכרון פרטי במקום ה‑150MB שתהליך WebView2 עולה.

אפשר להחזיק את שני החלונות פתוחים במקביל. שלושתם חולקים קובץ נתונים אחד
וקולטים את הרישומים אחד של השני תוך שניות ספורות.
...
מפרסם את `N+1`, ומצרף את המתקין ואת שלושת קבצי ה‑EXE. שום עדכון גרסה לא נכנס
...
src-tauri/
  ...
  agent/      SederPlusAgent.exe   — תהליך הרקע. בלי Tauri, ולכן בלי WebView
  shared/     קובץ הנתונים, היומן, וכללי תזכורות הרקע — ללא תלות ב‑Tauri
```
...
תחת הגדרות ← "יעדים והתראות" יש שני ערוצים וכמה כללים. הערוצים הם חלוניות
...
סיכום שבועי, אזהרה מוקדמת כשהקצב חורג, ותזכורת בתחילת החודש לדווח במערכת
הטלפונית על החודש שהסתיים. כולם חיים ב‑`src/lib/notifications.ts`.

הכללים נכתבו כדי לשאול "האם זה עדיין שווה אמירה עכשיו?" ולא כדי לירות בשעה
קבועה. כל אחד מהם פועל לכל היותר פעם ביום/שבוע/חודש, והניהול הזה שמור בקובץ
הנתונים המשותף כך שגם אם שני קובצי EXE פתוחים, אותה תזכורת לא תוצג פעמיים.

### גם כשהתוכנה סגורה

המתג "להזכיר גם כשהתוכנה סגורה" (כבוי כברירת מחדל) מפעיל את
`SederPlusAgent.exe`: תהליך בלי חלון ובלי WebView, שנרשם ב‑`HKCU\...\Run`
ועולה עם Windows. הוא מעיר שתי תזכורות בלבד — שסדר התחיל ועדיין לא נרשם
(בשמו, א׳ או ב׳), ושצריך לדווח במערכת הטלפונית על החודש שעבר, בימים 1–5 בשעה
20:00, עם כפתור "סמנתי שדיווחתי" על ההתראה עצמה.

הוא אינו מחליט מתי תזכורת נדרשת. האפליקציה מחשבת מראש, לשנה קדימה, את הדקה
שבה כל תזכורת מבשילה — לפי לוח השנה העברי, שעות הסדרים והרגלי ההגעה — וכותבת
זאת כ‑`backgroundPlan` (ראו `src/lib/background-plan.ts`); התהליך רק משווה שעון
מול המספרים האלה (`src-tauri/shared/src/plan.rs`). שני הצדדים חולקים את
`notificationsSent`, ולכן אף אחד מהם לא חוזר על מה שהשני כבר אמר. כיבוי המתג
מוחק את רישום ה‑Run, והתהליך שרץ מזהה זאת תוך דקה ונסגר מעצמו.
...
npm run test:rust    # מאגר הנתונים, היומן, העדכונים וכללי תזכורות הרקע
...
```bash
npm run exe          # -> release-win\SederPlus.exe, SederPlusQuick.exe, SederPlusAgent.exe
